...
//#define FCLK_SLOW() { MODIFY_REG(spiIfSd.spi->Instance->CR1, SPI_BAUDRATEPRESCALER_256, SPI_BAUDRATEPRESCALER_128); }	/* Set SCLK = slow, approx 280 KBits/s*/
//#define FCLK_FAST() { MODIFY_REG(spiIfSd.spi->Instance->CR1, SPI_BAUDRATEPRESCALER_256, SPI_BAUDRATEPRESCALER_8); }	/* Set SCLK = fast, approx 4.5 MBits/s */

#define CS_HIGH()	{spi_decs(&spiIF_SD);}
#define CS_LOW()	{spi_cs(&spiIF_SD);}
...
{
  BYTE rxDat;
  xSemaphoreTake(spiIF_SD.ready, portMAX_DELAY);
  spi_modify_presc(&spiIF_SD);
  HAL_SPI_TransmitReceive(spiIF_SD.spi, &dat, &rxDat, 1, 50);
  xSemaphoreGive(spiIF_SD.ready);
  return rxDat;
}
...
//		FCLK_FAST();			/* Set fast clock */
	  spiIF_SD.prescaler = SPI_BAUDRATEPRESCALER_8;
